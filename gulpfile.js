...
function webZip() {
  ...
  return gulp.src(tmpPath + '/**/*.*')
    ...
    .pipe(zip('june-cheque-app-v'+version+'-web.zip'))
...
function webExtensionZip() {
  ...
  return gulp.src(srcPath + '/**/*.*')
    .pipe(zip('june-cheque-app-v'+version+'-extension.zip'))
    .pipe(gulp.dest(distPath));
}

function webBuildSuccess(done) {
  var version = JSON.parse(fs.readFileSync('./package.json', 'utf8')).version;
  log(colors.green("Web artifact created at: 'dist/web/build/june-cheque-app-v" + version + "-web.zip'"));
  if (done) done();
}

function webExtBuildSuccess(done) {
  var version = JSON.parse(fs.readFileSync('./package.json', 'utf8')).version;
  log(colors.green("Web extension artifact created at: 'dist/web/build/june-cheque-app-v" + version + "-extension.zip'"));
  if (done) done();
}
